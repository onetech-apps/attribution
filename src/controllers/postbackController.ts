...
export class PostbackController {
    ...
    async handlePostback(req: Request, res: Response): Promise<void> {
        try {
            ...
            // Check if we have Facebook credentials (we can proceed without fbclid)
            if (!click.fb_id || !click.fb_token) {
                ...
                const responseBody = {
                    ...
                    click_found: true,
                    fb_tracking: false,
                    fb_reason: !click.fb_id ? 'Немає FB Pixel ID' : 'Немає FB Token'
                };
